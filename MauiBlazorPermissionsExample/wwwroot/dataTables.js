...
var Table=$('#'+options.TableID).DataTable({
	...
	function RowModal(row,data){
		...
		var editBtn = '';
		if(Table.canEdit  || (options.canEdit && options.canEdit(data))){
			editBtn = '<span class="zmdi zmdi-edit" style="font-size:16px;cursor:pointer;margin-left:20px;" id="editData_'+data.id+'" title="แก้ไขข้อมูล1"></span>';
		}
...
Editor.editRow = function(id,data,c,filter){
	...
	setTimeout(function(){
		Editor.edit($('#'+options.TableID+' tbody tr#'+id),'แก้ไขข้อมูล2 '+title);
	},500);
}
