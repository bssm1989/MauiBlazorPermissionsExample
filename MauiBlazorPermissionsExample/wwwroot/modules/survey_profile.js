	...
	var survey_profile = {
		...
		GIS:{
			addMarkers:function(data){
				...
				map.addLegend({id:'level1',title:'ทุนมนุษย์',type:'radio',checked:true,icon:'icons/pointLevel.png',parent:'level'});
				map.addLegend({id:'level2',title:'ทุนกายภาพ',type:'radio',checked:false,icon:'icons/pointLevel.png',parent:'level'});
				map.addLegend({id:'level3',title:'ทุนการเงิน',type:'radio',checked:false,icon:'icons/pointLevel.png',parent:'level'});
				map.addLegend({id:'level4',title:'ทุนธรรมชาติ',type:'radio',checked:false,icon:'icons/pointLevel.png',parent:'level'});
				map.addLegend({id:'level5',title:'ทุนทางสังคม',type:'radio',checked:false ,icon:'icons/pointLevel.png',parent:'level'});
				map.addLegend({id:'level6',title:'ทุนเฉลี่ย',type:'radio',checked:false ,icon:'icons/pointLevel.png',parent:'level'});
                    ...
                    handler.setInputAction(function(movement) {
                        ...
                        if ((Cesium.defined(pickedObject)) && (pickedObject.collection == entities)) {
                            ...
                            map.viewer.selectedEntity = new Cesium.Entity({
                                ...
                                edit:pickedObject.primitive.edit
                                // waiting add edit menu edit here 
                                // see pickedObject.primitive
                            });



                            console.log("yes");
                            var div = '<div class="modal" style="margin-top: 60px;"><div class="modal-dialog modal-lg" style="height: 400px;width:400px;max-width:800px;margin:4px;"><div class="modal-content" style="height: 100%;"><div class="modal-header" style="padding: 4px;"><button type="button" class="close" data-dismiss="modal">×</button></div><div class="modal-body" id="data_view" style="padding:4px;height: 96%;"></div></div></div></div>';
                            setTimeout(function() {
                                $(div).modal();
                                _view(keepIndex);
                            }, 100);
                            function _view(index) {
                                ...
                                var keepIndexFind=dataGroup2.findIndex(x=>x.HC==index);
								if(keepIndexFind<97786&&keepIndexFind!=-1){
									surveyText="survey";
									$('#data_view').html('<iframe id="dataiframe" class="responsive-iframe" style="display: block;width:95%;height: 100%;border: none;" src="https://www2.livingonnewpace.com/survey/show_hc62.php?hc=' + index.replace('"', '').replace('"', '') + '"></iframe>');
								}else{
	                                $('#data_view').html('<iframe id="dataiframe" class="responsive-iframe" style="display: block;width:95%;height: 100%;border: none;" src="https://www.livingonnewpace.com/survey_p2/show_hc62.php?hc=' + index.replace('"', '').replace('"', '') + '"></iframe>');								
								}
                                // https://www.livingonnewpace.com/survey/show_hc62.php?hc=1805-003158-6
//                                $('#data_view').html('<iframe id="dataiframe" class="responsive-iframe" style="display: block;width:95%;height: 100%;border: none;" src="../'+surveyText+'/show_hc62.php?hc=' + index.replace('"', '').replace('"', '') + '"></iframe>');
                                ...
                                // $('#data_view').html('<iframe id="dataiframe" class="responsive-iframe" style="margin-top: 60px;display: block;width:95%;height: 100%;border: none;" src="../survey/show_hc62.php?hc=1805-003158-6"></iframe>');
                                _hide();
                            }
                            function _hide() {
                ...
                $(".select-year").change(function() {
                    ...
                    if ($(".select-year").val() == "year_all") {
                        ...
                        indexEntityForSelectYear_start = 0;
                        indexEntityForSelectYear_end = 97786;

                    } else if ($(".select-year").val() == "year_2564") {
                        indexEntityForSelectYear_start = 97786;
                        indexEntityForSelectYear_end = entities.length;
                    }
                ...
                $("input").click(function() {
                    ...
                    if (entities.length > 0) {
                        ...
                        if (this.id == "survey_profile_a") {
                            ...
                            for (var i = indexEntityForSelectYear_start; i < indexEntityForSelectYear_end; i++) {
                                ...
                                //dataGroup2.showall=true;
								//dataIntersec.push(entities.get(i)._index,entities.get(i).id);
								dataGroup2[i].showall=true;
                    ...
                    for (let ii = 0; ii < $("input[type=checkbox]").length; ii++) {
                     ...
                     if (this.type == "checkbox" &&this.checked&& $("#survey_profile_a").is(":checked") || keepBoolean) {
                        ...
                        if (this.id.split("_").length == 2) {
                            ...
                            if(this.id.split("_")[1]<21) {
                                dataIntersec.push(this.id);
							var selectedYear=0;
							...
							if ($(".select-year").val() == "year_all") {
							  ...
							  selectedYear=1;
							  startEntitiesByYear=0;
								addRemoveEntitiesByYear=(entities.length-97786)*-1;

							} else if ($(".select-year").val() == "year_2564") {
								selectedYear=2;
								  startEntitiesByYear=97786;
								addRemoveEntitiesByYear=0;
							}
							...
							//let MaxEntitiseConYear=
								 $.each(r.data, function(i) {
									...
									for(let j=keep;j<countEntitise;j++){
										...
										if(this.HC==entities.get(j).id){
                                           // console.log(j);
											...
											keepBefore=this.HC;
                                            let showBool=false;
                                            if(dataGroup2[j].showall)
                                            showBool=true;
                                            entities.get(j).show=true;
                                            dataGroup2[j].showall=showBool;
											//console.log(showBool);
                                            if(showBool)countActiveInterSec++;
                                    ...
                                    console.log("interSec:"+countActiveInterSec);
                    ...
                    // False
                    if (this.type == "checkbox3" &&this.checked==false&& $("#survey_profile_a").is(":checked") || keepBoolean) {
                        console.log($("#survey_profile_a"));
                        console.log(this.id);
									...
									for(let j=keep;j<countEntitise;j++){
										...
										if(this.HC==entities.get(j).id){
                                           // console.log(j);
											...
											countEntitise=j;
											keepBefore=this.HC;
                                            let showBool=dataGroup2[j].showall?true:false;

                                            entities.get(j).show=showBool;
                                            dataGroup2[j].showall=showBool;
											//console.log(showBool);
                                            if(showBool)countActiveInterSec++;
										}else{
            ...
            addMarker: function(o) {
                ...
                function _add(dataByHc) {
                   ...
                   // if (dataByHc.photo) {
                    ...
                    var p = {
                        ...
                        description: {
                            ...
                            getValue: function() {
                                ...
                                if (!this.value) {
                                            ...
                                            if (databyHC) {
                                                ...
                                                if(databyHC.survey_year=='2564'){
                                                    DB.db({
                                                        ...
                                                        if (r.data && r.data[0]) {
                                                         //  debugger;
                                                         v.photo = '<div style="display:inline-block;">';
                                                        $.each(r.data, function() {
                                                           
                                                            v.photo = v.photo + '<img style="cursor:pointer;margin:2px;width:250px;" src="/survey_p2/picture/' + this.JUN + '/' + this.AMP + '/' + this.TMP + '/' + this.file_type + '" onclick="viewImage(\'/survey_p2/picture/' + this.JUN + '/' + this.AMP + '/' + this.TMP + '/' + this.file_type + '\')">';
                                                        ...
                                                        if(r.data[0].photo)  {
                                                            v.photo =  '<div style="display:inline-block;">';
                                                            $.each(JSON.parse(r.data[0].photo), function() {
                                                                v.photo = v.photo +  '<img style="cursor:pointer;margin:2px;width:250px;" src="../gis/' + this.path + '" onclick="viewImage(\'' + this.path + '\')">';
                                    ...
                                    var v = this.value;
                                    return v.hc + 'ที่อยู่ : ' + v.h_no + v.v_no + v.v_name + v.sub_district + v.district + v.province + v.postcode + v.location1 + v.member + v.person + v.tel + v.by + v.mistake+v.photo;
                                }
                        ...
                        view: function() {
                            ...
                            function _view() {
                                ...
                                //console.log(dataByHc.HC);
								var surveyText="survey_p2";
								if(dataGroup2.findIndex(x=>x.HC==dataByHc.HC)<97786){
									surveyText="survey";
	                                $('#data_view').html('<iframe id="dataiframe" class="responsive-iframe" style="width:100%;height: 100%;border: none;" src="https://www2.livingonnewpace.com/survey/index_gis.php?curr=show_hc6&hc=' + dataByHc.HC.replace('"', '').replace('"', '') + '"></iframe>');
								}else{
	                                $('#data_view').html('<iframe id="dataiframe" class="responsive-iframe" style="width:100%;height: 100%;border: none;" src="../'+surveyText+'/index.php?curr=show_hc6&hc=' + dataByHc.HC.replace('"', '').replace('"', '') + '"></iframe>');
								}
                    ...
                    entities._pointPrimitives[countNum].edit = p.edit;
                ...
                function createDescription() {
                    ...
                    if (!this.value) {
                        this.value = {
                            hc: 'รหัสบ้าน 22: ' + dataByHc.HC + '<br><br>',
                            h_no: '',
                            v_no: '',
                            v_name: '',
                            sub_district: '',
        ...
        Table: {
			...
			columnDefs: [
				...
				{targets:4,title:'แบบสอบถาม',width:'60px',
					render:function(d){
						return '<a href="/survey/survey_edit.php?HC='+d.HC+'" target="_blank" style="text-decoration:none;cursor:pointer;" aria-hidden="true" title="แบบสอบถาม">แบบสอบถาม</a>';
					}
				},
				{targets:5,title:'รูป',width:'200px',
					render:function(d){
						...
						if(d.photo)  {
                            debugger;
                            $.each(JSON.parse(d.photo), function() {
					...
					if(c.action=='edit'){
						$.each(b.data,function(){
							if(this.HC){
								...
								DB.db({table:'survey_b1'}).updateWhere(d,"HC='"+this.HC+"'");
							}
						});
					}
					if(c.action == 'remove'){
